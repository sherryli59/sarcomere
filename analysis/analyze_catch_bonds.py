    ...
    with h5py.File(h5file, "r") as fh:
        ...
        if data.size:
            for row in data:
                ...
                if myo_vel_ds is not None and am_bonds_ds is not None:
                    frame_myo_vel = myo_vel_ds[
                        min(step_idx, myo_vel_ds.shape[0] - 1)
                    ]
                    am_pairs = am_bonds_ds[
                        min(step_idx, am_bonds_ds.shape[0] - 1)
                    ]
