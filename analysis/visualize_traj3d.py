...
def plot_system(frame, data, myosin_length, actin_length, Lx, Ly, Lz, myosin_radius):
    ...
    plot_filaments_3d(actin_center, actin_direction, actin_length,
                  Lx, Ly, Lz, ax, color='k', color_spectrum=cb_strength*f_load)
    
    # Myosin data.
     # --- Filter to bonded myosins ---
    bonds = data["/myosin/bonds"][frame]
    valid_pairs = bonds[bonds[:, 0] >= 0].astype(int)

    if valid_pairs.size > 0:
        # Choose a colormap and normalize pair IDs
        cmap = plt.cm.get_cmap('tab20', len(valid_pairs))
        norm = mcolors.Normalize(vmin=0, vmax=len(valid_pairs) - 1)

        for pair_id, (iA, iB) in enumerate(valid_pairs):
            color = cmap(norm(pair_id))
            # Plot both myosins in the pair
            for idx in (iA, iB):
                center = data["/myosin/center"][frame][idx:idx+1]
                direction = data["/myosin/direction"][frame][idx:idx+1]
                plot_filaments_3d(center, direction, myosin_length,
                                  Lx, Ly, Lz, ax, color=color)
