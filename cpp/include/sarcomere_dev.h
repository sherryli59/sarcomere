...
class Sarcomere
{
    public:
        ...
        std::vector<std::vector<double>> actin_angular_forces_temp, myosin_angular_forces_temp;
        ...
        Sarcomere(int& n_actins, int& n_myosins, vector box0, double& actin_length, double& myosin_length,
            ...
            std::vector<std::vector<vec>> actin_forces_temp(n_threads, std::vector<vec>(n_actins, {0, 0}));
            std::vector<std::vector<vec>> myosin_forces_temp(n_threads, std::vector<vec>(n_myosins, {0, 0}));
            std::vector<std::vector<vec>> myosin_velocities_temp(n_threads, std::vector<vec>(n_myosins, {0, 0}));
            std::vector<std::vector<double>> actin_angular_forces_temp(n_threads, std::vector<double>(n_actins, 0));
            std::vector<std::vector<double>> myosin_angular_forces_temp(n_threads, std::vector<double>(n_myosins, 0));
        }
        ...
        void _set_to_zero(int& thread_id) {
        ...
        void _update2(int& thread_id) {
            ...
            for (int i = 0; i < actin.n; i++) {
                ...
                for (int index = 0; index < myosin_indices.size(); index++) {
                    ...
                    _set_cb(thread_id, i,cb_indices,cb_strengths);
                }
            }
        }

        void _update3(int& thread_id) {
            // Thread-local temporary lists for forces and velocities
            auto& local_actin_forces = actin_forces_temp[thread_id];
            auto& local_actin_angluar_forces = actin_angular_forces_temp[thread_id];
            auto& local_myosin_forces = myosin_forces_temp[thread_id];
            auto& local_myosin_velocities = myosin_velocities_temp[thread_id];
            auto& local_myosin_angular_forces = myosin_angular_forces_temp[thread_id];
        ...
        void _myosin_repulsion(int& thread_id, int& i, int& j){
        ...
        void _actin_repulsion(int& thread_id, int& i, int& j){
        ...
        void _set_cb(int& thread_id, int& i, int& j, double& normalized_strength, bool& add_connection){
            auto& local_actin_forces = actin_forces_temp[thread_id];
            auto& local_actin_angular_forces = actin_angular_forces_temp[thread_id];
            auto& local_myosin_forces = myosin_forces_temp[thread_id];
            auto& local_myosin_angular_forces = myosin_angular_forces_temp[thread_id];
        ...
        void _set_cb(int& thread_id, int& i, std::vector<int> indices, vector cb_strength){
            ...
            for (int index = 0; index < sorted_indices.size(); index++){
                ...
                add_connection = true;
                _set_cb(thread_id, i,j,cb_strength[sorted_indices[index]],add_connection);
            }
        }
