...
vec pbc_diff(const vec& a, const vec& b, const std::vector<double>& box, const std::array<bool,3>& periodic) {
    return utils::pbc_diff_masked(a, b, box, periodic);
}
...
double segment_segment_distance(const vec& A, const vec& B,
        ...
        const std::array<bool,3>& periodic)
    {
        ...
        shift.x = periodic[0] && Lx > 0.0 ? -Lx * std::round(displacement.x / Lx) : 0.0;
        shift.y = periodic[1] && Ly > 0.0 ? -Ly * std::round(displacement.y / Ly) : 0.0;
        shift.z = periodic[2] && Lz > 0.0 ? -Lz * std::round(displacement.z / Lz) : 0.0;
...
double segment_segment_distance(const vec& A, const vec& B,
        ...
        const std::vector<double>& box)
{
    static const std::array<bool,3> all_periodic{true, true, true};
...
{
    static const std::array<bool,3> all_periodic{true, true, true};
    return segment_segment_distance_w_normal(A, B, C, D, box, all_periodic);
}
...
void apply_pbc(vec& actin_left, vec& actin_right,
    ...
    static const std::array<bool,3> all_periodic{true, true, true};
    apply_pbc(actin_left, actin_right, myosin_left, myosin_right, box, all_periodic);
}
...
std::tuple<double, vec, vec> subsegment_within_distance(vec A, vec B, vec C, vec D, double d, std::vector<double> box, const std::array<bool,3>& periodic) {
    apply_pbc(A, B, C, D, box, periodic);
...
std::tuple<double, vec, vec> subsegment_within_distance(vec A, vec B, vec C, vec D, double d, std::vector<double> box) {
    static const std::array<bool,3> all_periodic{true, true, true};
    return subsegment_within_distance(A, B, C, D, d, box, all_periodic);
}
...
am_interaction analyze_am(vec actin_left, vec actin_right,
    ...
    interaction.crosslinkable_ratio = (interaction.crosslinkable_end-actin_left).norm()/actin_right.distance(actin_left, box, periodic);
...
am_interaction analyze_am(vec actin_left, vec actin_right,
    ...
    static const std::array<bool,3> all_periodic{true, true, true};
    return analyze_am(actin_left, actin_right, myosin_left, myosin_right, d, box, all_periodic);
}
